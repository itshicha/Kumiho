...
@bot.tree.command(name="timeout", description="إسكات عضو لفترة محددة (استخدم m للدقائق و h للساعات)")
...
async def slash_timeout(interaction: discord.Interaction,
    ...
    try:
        ...
        return await interaction.response.send_message("❌ صيغة الوقت غلط! استخدم مثلاً `10m` أو `2h`.", ephemeral=True)
...
import datetime
from dateutil.relativedelta import relativedelta # تأكد من وجود هذه المكتبة في requirements.txt

@bot.command(name="طير")
@commands.has_permissions(ban_members=True)
async def cmd_ban(ctx, member: discord.Member, duration: str = "0d", *, reason="غير محدد"):
    ...
    if unit == 'd':
        ...
        return await ctx.send("❌ وحدة الوقت غير معروفة! استخدم (d للأيام، m للشهور، y للسنوات)")
    ...
    if log_id:
        ...
        if log_channel:
            log_embed = discord.Embed(
                title="🔨 سجل العقوبات: طيران",
                color=discord.Color.red(),
                timestamp=now
            )
            log_embed.add_field(name="المشرف:", value=ctx.author.mention, inline=True)
            log_embed.add_field(name="العضو المعاقب:", value=member.mention, inline=True)
            log_embed.add_field(name="المدة:", value=duration_text, inline=True)
            log_embed.add_field(name="تاريخ الانتهاء التقديري:", value=end_date.strftime("%Y/%m/%d"), inline=True)
    ...
    if log_id:
        ...
        if log_channel:
            try:
                ...
                await log_channel.send(embed=log_embed)
                print(f"✅ تم إرسال اللوق بنجاح لروم: {log_channel.name} في سيرفر {ctx.guild.name}")
            except Exception as e:
    ...
    if log_id:
        ...
        if log_channel:
            try:
                ...
                log_embed.add_field(name="العدد المحذوف:", value=f"{amount} رسالة", inline=True)
        ...
        SERVER_VOICE_LOGS = {
            ...
            if log_id:
                ...
                if log_channel:
                    ...
                    embed.add_field(name="العضو المنقول:", value=member.mention, inline=True)
            ...
            await interaction.response.send_message(f"❌ فشل النقل: {e}", ephemeral=True)
...
async def on_message_delete(message):
    ...
    if log_id:
        ...
        if log_channel:
            ...
            embed.add_field(name="المحتوى", value=f"```\n{message.content or 'لا يوجد محتوى نصي'}\n```", inline=False)
            embed.set_footer(text=f"سيرفر: {message.guild.name} | معرف المستخدم: {message.author.id}")
...
async def on_message_edit(before, after):
    ...
    if log_id:
        ...
        if log_channel:
            ...
            embed.set_footer(text=f"سيرفر: {before.guild.name} | معرف المستخدم: {before.author.id}")
...
async def on_guild_channel_create(channel):
    if isinstance(channel, discord.VoiceChannel): return
    ...
    if log_id:
        ...
        if log_channel:
            ...
            entry = None
            async for e in channel.guild.audit_logs(action=discord.AuditLogAction.channel_create, limit=1):
                entry = e
                break
...
async def on_guild_channel_update(before, after):
    if isinstance(after, discord.VoiceChannel): return
...
@bot.event
async def on_guild_role_create(ro�le):
    SERVER_ROLE_LOGS = {
    ...
    if log_id:
        ...
        if log_channel:
            entry = None
            async for e in role.guild.audit_logs(action=discord.AuditLogAction.role_create, limit=1):
                entry = e
                break
            user_name = entry.user.mention if entry else "غير معروف"
...
async def on_guild_role_update(before, after):
    ...
    # 2. حالة تعديل الصلاحيات (Permissions)
        ...
        if added_perms:
            embed.add_field(name="✅ صلاحيات أُضيفت:", value=", ".join(added_perms).replace("_", " "), inline=False)
        if removed_perms:
            embed.add_field(name="❌ صلاحيات أُزيلت:", value=", ".join(removed_perms).replace("_", " "), inline=False)
        ...
        for old_inv in invites_before:
            for new_inv in invites_after:
                if old_inv.code == new_inv.code and old_inv.uses < new_inv.uses:
                    ...
                    total_invites = sum(i.uses for i in invites_after if i.inviter.id == old_inv.inviter.id)
                    break
...
async def on_guild_channel_update(before, after):
    ...
    embed.set_footer(text=f"{after.guild.name} | Today at {datetime.datetime.now().strftime('%I:%M %p')}")
    ...
    if before.name != after.name:
        ...
        embed.add_field(name="Old Name:", value=f"```\n{before.name}\n```", inline=False)
    ...
    # 2. حالة تغيير الصلاحيات (Permissions) مثل قفل الروم أو فتحه
        ...
        for target, overwrite in after.overwrites.items():
            ...
            if old_overwrite != overwrite:
                ...
                pe�rm_details = f"↘️ **{target_mention}**\n{status}"
                break
    ...
    for target, overwrite in after.overwrites.items():
        if isinstance(target, discord.Member):
            ...
            if old_ov is None:
                ...
                embed.description = f"تم إضافة **{target.mention}** إلى صلاحيات القناة: {after.mention}"
    ...
    if not found_member_change:
        for target, old_ov in before.overwrites.items():
            if isinstance(target, discord.Member) and target not in after.overwrites:
                ...
                embed.description = f"تم إزالة **{target.mention}** من صلاحيات القناة: {after.mention}"
                found_member_change = True
                break
...
async def on_guild_channel_update(before, after):
    ...
    if ov_before.connect != ov_after.connect:
        ...
        status_text = "Everyone can join" if ov_after.connect else "Only trusted users can join"
        
    elif ov_before.view_channel != ov_after.view_channel:
        action_name = "👁️ Channel Visible" if ov_after.view_channel else "🚫 Channel Invisible"
        status_text = "Everyone can view" if ov_after.view_channel else "Only trusted users can view"

    elif ov_before.send_messages != ov_after.send_messages:
        action_name = "🗨️ Chat Opened" if ov_after.send_messages else "🚫 Chat Closed"
        status_text = "Everyone can text" if ov_after.send_messages else "Only trusted users can text"

    embed.title = action_name
    embed.description = f"**Channel:** {after.mention}\n**Status:** {status_text}"
    embed.add_field(name="Responsible Moderator:", value=responsible_mod.mention, inline=False)
...
async def on_message(message):
    ...
    if g_id not in levels: levels[g_id] = {}
    if u_id not in levels[g_id]: levels[g_id][u_id] = {"xp": 0, "voice_xp": 0, "level": 1}
    ...
    for guild in bot.guilds:
        for vc in guild.voice_channels:
            for member in vc.members:
                if member.bot or member.voice.self_deaf: continue
                g_id, u_id = str(guild.id), str(member.id)
                if g_id not in levels: levels[g_id] = {}
                if u_id not in levels[g_id]: levels[g_id][u_id] = {"xp": 0, "voice_xp": 0, "level": 1}
                levels[g_id][u_id]["voice_xp"] = levels[g_id][u_id].get("voice_xp", 0) + 10
    save_levels(levels)
...
async def rank(ctx, member: discord.Member = None):
    member = member or ctx.author
    levels = load_levels()
    stats = levels.get(str(ctx.guild.id), {}).get(str(member.id), {"xp": 0, "voice_xp": 0, "level": 1})
    
    lvl = stats["level"]
    chat_xp, voice_xp = stats["xp"], stats.get("voice_xp", 0)
    needed_xp = lvl * 200

    try: background = Editor("rank_bg.png").resize((900, 450))
    except: background = Editor(Canvas((900, 450), color="#1a1a1a"))
    ...
    background.bar((280, 210), max_width=550, height=35, percentage=(chat_xp/needed_xp)*100, fill="#ffad14")
    background.text((280, 180), f"CHAT XP: {chat_xp}/{needed_xp}", font=font_s, color="#FFFFFF")

    background.bar((280, 310), max_width=550, height=35, percentage=(voice_xp/needed_xp)*100, fill="#e67e22")
    background.text((280, 280), f"VOICE XP: {voice_xp}/{needed_xp}", font=font_s, color="#FFFFFF")
...
async def top(ctx):
    ...
    # جلب أفضل 10 كتابي
    sorted_text = sorted(levels[g_id].items(), key=lambda x: x[1].get('xp', 0), reverse=True)[:10]
    text_desc = ""
    for i, (uid, data) in enumerate(sorted_text):
        text_desc += f"**#{i+1}** | <@{uid}> - `{data.get('xp', 0)}` XP\n"

    # جلب أفضل 10 صوتي
    sorted_voice = sorted(levels[g_id].items(), key=lambda x: x[1].get('voice_xp', 0), reverse=True)[:10]
    voice_desc = ""
    ...
    embed.add_field(name="💬 أفضل 10 كتابي", value=text_desc or "لا توجد بيانات", inline=True)
    embed.add_field(name="🎙️ أفضل 10 صوتي", value=voice_desc or "لا توجد بيانات", inline=True)
...
async def top_text(ctx):
    levels = load_levels()
    g_id = str(ctx.guild.id)
    if g_id not in levels: return await ctx.send("لا توجد بيانات.")
    ...
    await ctx.send(embed=discord.Embed(title="💬 متصدري الكتابة", description=desc, color=0x3498db))
...
async def top_voice(ctx):
    ...
    sorted_users = sorted(levels[g_id].items(), key=lambda x: x[1].get('voice_xp', 0), reverse=True)
    desc = ""
    for i, (uid, data) in enumerate(sorted_users[:10]):
        desc += f"**#{i+1}** | <@{uid}> - Voice XP: `{data.get('voice_xp', 0)}`\n"
    
    await ctx.send(embed=discord.Embed(title="🎙️ متصدري الفويس", description=desc, color=0x2ecc71))
...
async def setlevel(ctx, member: discord.Member, level: int):
    ...
    if added: msg += f" ومنحه رتب: {', '.join(added)}"
    await ctx.send(msg)
...
async def setxp_error(ctx, error):
    if isinstance(error, commands.MissingPermissions):
        ...
        await ctx.send("⚠️ خطأ في المدخلات: تأكد من منشن العضو ثم كتابة الرقم (مثال: `!setxp @user 5000`)")
        
# --- [7] التشغيل ---
@bot.event
async def on_ready():
    if not voice_xp_handler.is_running(): voice_xp_handler.start()
...
async def on_member_update(before, after):
    ...
    # التحقق من إزالة رتبة
        ...
        embed.set_author(name=after.name, icon_url=after.display_avatar.url)
        embed.add_field(name="الرتبة التي أُزيلت:", value=removed_role.mention, inline=False)
        embed.add_field(name="بواسطة:", value=responsible_mod, inline=False)
        embed.set_footer(text=f"إدارة الرتب | {after.guild.name}")
    ...
    async for entry in after.guild.audit_logs(action=discord.AuditLogAction.role_update, limit=1):
        if entry.target.id == after.id:
            ...
            if before.permissions != after.permissions:
                ...
                if added_perms:
                    embed.add_field(name="✅ صلاحيات أُعطيت:", value=", ".join(added_perms).replace("_", " "), inline=False)
                if removed_perms:
                    embed.add_field(name="❌ صلاحيات سُحبت:", value=", ".join(removed_perms).replace("_", " "), inline=False)

            # 2. فحص تغير اسم الرتبة
            if before.name != after.name:
                embed.add_field(name="📝 تغيير الاسم:", value=f"من **{before.name}** إلى **{after.name}**", inline=False)

            # 3. فحص تغير لون الرتبة
            if before.color != after.color:
                embed.add_field(name="🎨 تغيير اللون:", value=f"من **{before.color}** إلى **{after.color}**", inline=False)
...
async def on_guild_channel_create(channel):
    ...
    SERVER_LOGS = {1394284974680838388: 1479861118137794684, 1182934425013604362: 1480697749774139453}
    log_id = SERVER_LOGS.get(channel.guild.id)
    if not log_id: return
    log_channel = bot.get_channel(log_id) or await bot.fetch_channel(log_id)

    async for entry in channel.guild.audit_logs(action=discord.AuditLogAction.channel_create, limit=1):
        embed = discord.Embed(title="🆕 إنشاء قناة كتابية جديدة", color=discord.Color.green(), timestamp=datetime.datetime.now())
...
async def on_guild_channel_delete(channel):
    ...
    async for entry in channel.guild.audit_logs(action=discord.AuditLogAction.channel_delete, limit=1):
        embed = discord.Embed(title="🗑️ حذف قناة كتابية", color=discord.Color.red(), timestamp=datetime.datetime.now())
...
@bot.event
async def on_guild_channel_update(before, after):
    if not isinstance(after, discord.TextChannel): return
    SERVER_LOGS = {1394284974680838388: 1479861118137794684, 1182934425013604362: 1480697749774139453}
    log_id = SERVER_LOGS.get(after.guild.id)
    if not log_id: return
    log_channel = bot.get_channel(log_id) or await bot.fetch_channel(log_id)
    ...
    async for entry in after.guild.audit_logs(action=discord.AuditLogAction.channel_update, limit=1):
        if entry.target.id == after.id:
            embed = discord.Embed(title=f"⚙️ تعديل في القناة: {after.name}", color=discord.Color.blue(), timestamp=datetime.datetime.now())
            ...
            if before.topic != after.topic:
                embed.add_field(name="📖 تغيير الوصف:", value=f"من `{before.topic or 'لا يوجد'}` إلى `{after.topic or 'لا يوجد'}`", inline=False)

            if before.overwrites != after.overwrites:
                embed.add_field(name="🔐 تغيير الصلاحيات:", value="تم تعديل الأذونات الخاصة بالقناة (قفل/فتح/صلاحيات أعضاء)", inline=False)
...
async def on_guild_channel_create(channel):
    ...
    SERVER_VOICE_LOGS = {1394284974680838388: 1479861318331793410, 1182934425013604362: 1480697888257609778}
    log_id = SERVER_VOICE_LOGS.get(channel.guild.id)
    if not log_id: return
    log_channel = bot.get_channel(log_id) or await bot.fetch_channel(log_id)

    async for entry in channel.guild.audit_logs(action=discord.AuditLogAction.channel_create, limit=1):
        embed = discord.Embed(title="🔊 إنشاء قناة صوتية جديدة", color=discord.Color.green(), timestamp=datetime.datetime.now())
...
async def on_guild_channel_delete(channel):
    ...
    log_id = SERVER_VOICE_LOGS.get(channel.guild.id)
    if not log_id: return
    log_ch�annel = bot.get_channel(log_id) or await bot.fetch_channel(log_id)

    async for entry in channel.guild.audit_logs(action=discord.AuditLogAction.channel_delete, limit=1):
        embed = discord.Embed(title="🗑️ حذف قناة صوتية", color=discord.Color.red(), timestamp=datetime.datetime.now())
...
@bot.event
async def on_guild_channel_update(before, after):
    if not isinstance(after, discord.VoiceChannel): return
    SERVER_VOICE_LOGS = {1394284974680838388: 1479861318331793410, 1182934425013604362: 1480697888257609778}
    log_id = SERVER_VOICE_LOGS.get(after.guild.id)
    if not log_id: return
    log_channel = bot.get_channel(log_id) or await bot.fetch_channel(log_id)
    ...
    async for entry in after.guild.audit_logs(action=discord.AuditLogAction.channel_update, limit=1):
        if entry.target.id == after.id:
            embed = discord.Embed(title=f"⚙️ تعديل إعدادات: {after.name}", color=discord.Color.blue(), timestamp=datetime.datetime.now())
            
            if before.name != after.name:
                embed.add_field(name="📝 تغيير الاسم:", value=f"من `{before.name}` إلى `{after.name}`", inline=False)
            
            if before.user_limit != after.user_limit:
                embed.add_field(name="👥 سعة الروم:", value=f"من `{before.user_limit or '∞'}` إلى `{after.user_limit or '∞'}`", inline=False)

            if before.bitrate != after.bitrate:
                embed.add_field(name="🎙️ جودة الصوت:", value=f"من `{before.bitrate//1000}kbps` إلى `{after.bitrate//1000}kbps`", inline=False)

            if before.overwrites != after.overwrites:
                embed.add_field(name="🔐 الأذونات:", value="تم تعديل صلاحيات (المنع/السماح) في الروم", inline=False)
    ...
    if before.channel and after.channel and before.channel != after.channel:
        async for entry in member.guild.audit_logs(action=discord.AuditLogAction.member_move, limit=1):
            if entry.target.id == member.id:
                embed = discord.Embed(title="🚚 نقل عضو يدوياً", color=discord.Color.dark_gold(), timestamp=datetime.datetime.now())
...
async def on_voice_state_update(member, before, after):
    ...
    # 2. حالة الخروج من الفويس
        ...
        embed.add_field(name="الروم الذي غادره:", value=f"🔊 {before.channel.name}", inline=True)
...
async def on_guild_role_create(role):
    ...
    async for entry in role.guild.audit_logs(action=discord.AuditLogAction.role_create, limit=1):
        embed = discord.Embed(title="✨ إنشاء رتبة جديدة", color=discord.Color.green(), timestamp=datetime.datetime.now())
...
@bot.event
async def on_guild_role_delete(role):
    SERVER_ROLE_LOGS = {1394284974680838388: 1479861118137794684, 1182934425013604362: 1480697982671388845}
    log_id = SERVER_ROLE_LOGS.get(role.guild.id)
    if not log_id: return
    log_channel = bot.get_channel(log_id) or await bot.fetch_channel(log_id)

    async for entry in role.guild.audit_logs(action=discord.AuditLogAction.role_delete, limit=1):
        embed = discord.Embed(title="🗑️ حذف رتبة", color=discord.Color.red(), timestamp=datetime.datetime.now())
...
async def on_guild_role_update(before, after):
    ...
    async for entry in after.guild.audit_logs(action=discord.AuditLogAction.role_update, limit=1):
        if entry.target.id == after.id:
            embed = discord.Embed(title=f"⚙️ تعديل في رتبة: {after.name}", color=discord.Color.blue(), timestamp=datetime.datetime.now())
            ...
            if before.permissions != after.permissions:
                embed.add_field(name="🔐 تغيير الصلاحيات:", value="تم تعديل أذونات الرتبة", inline=False)
...
async def auto_reply_system(message):
    ...
    # 2. الرد على السلام عليكم
        ...
        embed = discord.Embed(description="**وعليكم السلام ورحمة الله وبركاته، نورت السيرفر يا غالي** ✨", color=discord.Color.green())
        await message.reply(embed=embed)

    # 3. الرد على صباح الخير
    elif "صباح الخير" in content:
        embed = discord.Embed(description="**صباح النور والسرور، يومك سعيد ومليء بالإنجازات** ☀️", color=0xffd700)
        await message.reply(embed=embed)

    # 4. الرد على مساء الخير
    elif "مساء الخير" in content:
        embed = discord.Embed(description="**مساء الورد والأنوار، نورتنا بطلتك المسائية** ✨", color=0x5865f2)
        await message.reply(embed=embed)
